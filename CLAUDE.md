...
- **`get_weibo_detail` 必须带 `isGetLongText=true`**：对 `isLongText=true` 的长微博，不带该参数时服务端只返回截断版 `text`/`text_raw`（实测 155→212 字符）。已验证与 `/ajax/statuses/longtext` 端点等价，故未引入该端点。改该接口时不要去掉这个参数（见 `docs/troubleshooting.md` 2026-07-11 条）。
- **渲染层显示作者 uid**：`render_weibo_card`（search/feed/home 列表）的 `@name` 行带 `uid=<idstr|id>`，`detail` 渲染带 `UID: <uid>` 单独行——为打通「列表/详情 → `weibo profile <uid>`/`weibo weibos <uid>`」链路。注意移动端 search 的 `mblog.user` 只有 `id`、无 `idstr`，故渲染取值用 `idstr`→`id` 回退。`source` 字段渲染前过 `strip_html`（带链接来源会含 `<a>`）。
